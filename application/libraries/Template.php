...
class Template
{
	...
	private $_metadataCss = array();
	private $_metadataJs = array();
	 ...
	 * @return	void
	 */
	public function build($view, $data = array(), $return = FALSE, $isCms = false)
	{
		...
		$template['metadataCss'] = '';
		for ($i = 0; $i < count($this->_metadataCss); $i++)
			$template['metadataCss'] .= '<link rel="stylesheet" type="text/css" media="all" href="'.$this->_metadataCss[$i].'" />';

		$template['metadataJs'] = '';
		for ($i = 0; $i < count($this->_metadataJs); $i++)
			$template['metadataJs'] .= '<script src="'.$this->_metadataJs[$i].'"></script>';
	...
	private function _find_view_folder()
	{
		...
		// Base view folder
		$view_folder = APPPATH.'modules/'.($this->_isCms ? 'cms' : 'comum').'/views/';
	...
	private function _load_view($view, array $data, $parse_view = TRUE, $override_view_path = NULL)
	{
		...
		if ($override_view_path !== NULL)
		{
			...
			{//die($override_view_path.$view.self::_ext($view));
				// Load content and pass through the parser
				// die($override_view_path);
				$content = $this->_ci->parser->parse_string($this->_ci->load->file(
					'application/modules/'.$view.self::_ext($view),
